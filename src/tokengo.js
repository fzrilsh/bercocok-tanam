...
async function runTokenGoHttpPhases(account, axiosInstance, oauthResult, proxy, log, updateProgress, affCode = null) {
    const { code, state, oauthState, stateCookies } = oauthResult;
    ...
    const apiKey = await revealApiKey(axiosInstance, tokenId, sessionCookie, userId, log);
...
async function processTokenGoOnBrowser(account, ctx) {
    const { page, proxy, log, updateProgress, affCode = null } = ctx;
    const axiosInstance = createAxiosInstance(proxy, log);

    updateProgress({ step: "Harvesting state", email: account.email });
    const phase0Result = await harvestOAuthState(axiosInstance, log, affCode);
    const oauthState = phase0Result.state;
    const stateCookies = phase0Result.cookies;
    ...
    // Do NOT close browser — orchestrator owns it
    return runTokenGoHttpPhases(
        account,
        axiosInstance,
        { code, state, oauthState, stateCookies },
        proxy,
        log,
        updateProgress,
        affCode,
...
async function processTokenGoAccountOnce(
    ...
    try {
        ...
        try {
            ...
            newAffCode = await runTokenGoHttpPhases(
                account,
                axiosInstance,
                { code, state, oauthState, stateCookies },
                proxy,
